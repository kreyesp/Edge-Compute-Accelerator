...
def main():
    ...
    # --- Plotting ---------------------------------------------
    print('\nGenerating plots...')
    PLOT_TOP_N = 20  # at the top with other tunables

    try:
        fig_energy, _ = plot_energy_breakdown(
            [best_mapping], ['einsum', 'component'], ['action'],
            ['Best EDP Mapping']
        )
        fig_energy.suptitle('Energy Breakdown (Tensix NEO Edge + TinyYOLOv2)')
        fig_energy.tight_layout()
        fig_energy.savefig('custom_accel_energy_breakdown.png', bbox_inches='tight')
        print("  Saved 'custom_accel_energy_breakdown.png'")

        fig_lat, _ = plot_latency_comparison(
            [mappings], ['All Mappings']
        )
        fig_lat.suptitle('Latency Comparison (Tensix NEO Edge + TinyYOLOv2)')
